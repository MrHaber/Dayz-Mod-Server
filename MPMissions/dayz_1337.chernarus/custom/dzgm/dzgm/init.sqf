...
keyboard_keys set [DIK_LWIN,{if (tagName) then {tagName = false;titleText ["\n\nТэги группы отключены","PLAIN DOWN"];titleFadeOut 4;} else {tagName = true;titleText ["\n\nТэги группы включены","PLAIN DOWN"];titleFadeOut 4;};_handled = true;}];
...
[] spawn dzgmInit;
[] spawn dzgmSlowLoop;
systemChat "Нажмите правой кнопкой мыши по радио в инвентаре что-бы открыть панель групп";